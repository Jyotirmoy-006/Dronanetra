...
export function buildPropellerAssembly(materials) {
  ...
  // ── SPINNER NOSE CONE (AERODYNAMIC PARABOLIC CONE POINTING FORWARD +X) ──
  // Profile curve: Tip at (0.001, 0.165) -> Base at (0.056, 0.000)
  const spinnerPts = [];
  const segs = 24;
  for (let i = 0; i <= segs; i++) {
    const t = i / segs; // 0 at tip, 1 at base
    const y = 0.165 * (1.0 - t); // y goes 0.165 -> 0.0
    // Parabolic aerodynamic bullet profile
    const r = 0.056 * Math.sqrt(t) * (1.0 + 0.15 * Math.sin(t * Math.PI));
    spinnerPts.push(new THREE.Vector2(Math.max(0.0008, r), y));
  }
  // Base inward lip
  spinnerPts.push(new THREE.Vector2(0.025, 0.000));
  ...
  const pinstripeGeo = new THREE.TorusGeometry(0.044, 0.0016, 8, 40);
  ...
  pinstripe.position.set(0.055, 0, 0);
  group.add(pinstripe);

  // Silver base accent ring at backplate interface
  const baseTrimGeo = new THREE.TorusGeometry(0.056, 0.0018, 8, 40);
  ...
  baseTrim.position.set(0.005, 0, 0);
  group.add(baseTrim);

  // ── 6 PERIMETER RETENTION SCREWS AROUND SPINNER BASE (exp_right.jpg) ──
  for (let a = 0; a < Math.PI * 2; a += Math.PI / 3) {
    const screwGeo = new THREE.CylinderGeometry(0.0018, 0.0018, 0.004, 8);
    ...
    screw.position.set(0.015, Math.sin(a) * 0.052, Math.cos(a) * 0.052);
    group.add(screw);
  }

  // ── CENTRAL RETENTION WASHER & HEX NUT (exp_right.jpg) ──
  const tipWasherGeo = new THREE.CylinderGeometry(0.008, 0.008, 0.003, 16);
  ...
  tipWasher.position.set(0.168, 0, 0);
  group.add(tipWasher);

  const tipNutGeo = new THREE.CylinderGeometry(0.006, 0.006, 0.006, 6);
  ...
  tipNut.position.set(0.172, 0, 0);
  group.add(tipNut);

  // ── SPINNER REAR BULKHEAD DISC ──
  const bulkheadGeo = new THREE.RingGeometry(0.020, 0.056, 32);
